...
class ThreatType extends CActiveRecord {
    ...
    public function constructStreamGraphTable($routes) {
        Yii::app()->db->createCommand()->truncateTable('temp_streamgraph');
        ...
        Yii::app()->db->createCommand(rtrim($sql, ','))->queryAll();
    }
    ...
    public function getNodeLinkData($routes, $waypoints, $threshold, $date) {
        ...
        Yii::app()->db->createCommand($sql_insert)->queryAll();
        ...
        $results = Yii::app()->db->createCommand($sql_select)->queryAll();

        //DELETE data from the temp table
        $sql_delete = 'DELETE FROM "temp_NodeLink"';
        Yii::app()->db->createCommand($sql_delete)->queryAll();
        return $results;
    }
    ...
    public function getSparklineData($routes, $date, $threshold) {
        ...
        foreach ($routes as $route) {
            ...
            $results_heatmap = Yii::app()->db->createCommand($sql_heatmap)->queryAll();
            $overall_max = -1;
            $agg_max = -1;
            $output = array();
            ...
            $results_sparklines = Yii::app()->db->createCommand($sql_sparklines)->queryAll();
    ...
    public function constructWaypointTable($routes) {
        Yii::app()->db->createCommand()->truncateTable('waypoints2');
    ...
    public function getCountAndType($routes) {
        Yii::app()->db->createCommand()->truncateTable('temp_points');
        ...
        $results = Yii::app()->db->createCommand(rtrim($sql, ','))->queryAll();
